...
制約環境に投げるプロンプトで **裸の「見積」は使わない**。業務文脈では開発工数・実装規模の見積と読めるため、
受け取った Claude が仕様を読み込んで実装方針の検討を始める — **避けたかった探索そのものが起動する**。

- ❌ 「{機能名} まわりの見積を取りたい」
- ✅ 「これは【トークン消費量の事前計測】です。開発工数・実装規模の見積ではありません」を **1 行目に置く**

加えて、禁止事項に誤読ルートを明示的に列挙して二重化する（1 行目を読み飛ばしても止まる）:

```
- 開発工数・スケジュール・実装方針を検討する（今回の目的ではありません）
- 仕様の内容を分析・要約する（測るのはコード量だけです）
```

同様に、プロンプト中の見出しも「見積」ではなく「トークン計測」と書く。曖昧な語を 1 つ残すだけで
数十万トークン級の逸走を招くため、**制約環境向けの語彙は「何を測るか」まで書き切る**のを原則とする。
...
### 3.2 module はモジュール名から推測せず、識別子 grep で特定する

`--modules` に何を渡すかを module 名の見た目から推測すると外れる（モノレポではサービス名と実装場所が
一致しないことが普通）。**仕様書に出てくる固有の識別子を grep する**方が正確で、かつ安い —
`grep -rl` はパスしか返さないため出力が小さい。Step V1 §3.e の経路 2 を先回りする形になる。

拾う識別子は、仕様書側に書かれていて推測が要らないものに限る（API パス、エラーコード、
定数名、camelCase のフィールド名など）。ヒットをディレクトリ単位に集計すると module がそのまま見える:

```bash
grep -rl -F -e '{識別子1}' -e '{識別子2}' … \
  --exclude-dir={node_modules,.git,build,dist,out,vendor,Pods,.venv,__pycache__} \
  artifacts/{app_name}/input-sources/ \
  | sed 's|/[^/]*$||' | sort | uniq -c | sort -rn | head -25
```

- `-F`（リテラル一致）+ `-e` の併記は必須 — 識別子に正規表現メタ文字（`.` `(` `[` `/`）が混ざると
  誤 hit / 取り落としが起きる。
- `--exclude-dir` を付けるのは、`git clone` で持ち込んだツリーに依存ディレクトリが残っている場合に
  hit が `head` で切られて肝心の呼び出し側が落ちるのを防ぐため。
- ⚠️ **grep の hit は「範囲候補」であって根拠ではない**。突合の根拠になるのは Step V2 が実際に読んだ
  `file:line` だけ（`skills/reverse-verify/02-targeted-crosscheck/SKILL.md`）。

この識別子リストは Step V1 の intake（対象の記述）にもそのまま流用する。ユーザーの言葉として渡せば
出所のあるキーワードとして扱われ、範囲導出が当たる（推測でキーワードを膨らませるのは V1 §2 が禁じている）。
